...
$S+S^2/2$, reaching $3/2$ at iSWAP.  In the intervening branch the value is
...
control: it exhibits three analytic regimes, with an algebraic quartic in the
middle.  A dedicated search found no previous evaluation of
...
$B=1/2-A$, and let $\lambda$ be the largest eigenvalue of $M_3(A)$.  Put
\[
...
\begin{equation}
 P(\lambda;A,u)=\lambda^3-A(1-u)\lambda^2
 -(ABm+B^2N)\lambda-AB^2\Theta.
...
$4\lambda$ and $P_\lambda>0$.

For fixed channel parameters, the diamond-SDP value is concave in its input
density: the feasible pairs $(\rho,Y)$ in the Watrous primal form a convex
set and the objective is linear.  Since the invariant density is affine in
$A$, the objective is concave in $A$.  Its maximizer is unique, since
otherwise an interval of maximizing $A$ values would make the cubic
$P(\lambda;A,u)$ vanish identically despite its leading coefficient
$-\Theta$.  The resulting facet objective $f(u)$ is convex and continuous.
Its endpoint derivatives point inward, and its minimizer has
$0<u<1$ and $0<A<1/2$ in the intermediate interval.  At $A=0$ the only
possible stationary value is $u=C^2$, where the inward derivative has sign
\[
 C(1-C)S[2SC-(3C^3+C^2-C-1)],
\]
which changes precisely at $\theta_1$.

A flat interval of minimizing $u$ values is impossible.  Indeed,
\[
 \operatorname{disc}_A P
 =\frac{(C-1)^2\lambda^2(2\lambda+u)}{16}R(u,\lambda,C),
 \qquad [u^7]R=(1-C)^2-8\lambda.
\]
...
Constancy on an interval would force $R$ to vanish identically in $u$, hence
$4\lambda=(1-C)^2/2<1/2$, whereas the $A=0$ witness gives
$4\lambda\geq2SC>1/2$.  The facet saddle is therefore unique and varies
continuously.
...
\[
 P_u=2ABG,\qquad
 G=(C-u)\lambda+B\Psi,\qquad
 \Psi=u^2-C(1+C)u+C.
\]
Here $\Psi>0$ because its discriminant is
$C(C-1)(C^2+3C+4)<0$.  Eliminating $A,u,\lambda$ from
...
limiting weak saddle has $A=0$, $u=C^2$, $\lambda=SC/2$, and
\[
 G=\frac{C(1-C)}2[(1+C)+SC]>0.
\]
Therefore $G>0$ throughout $(\theta_1,\theta_2)$.  The gradient of the
actual convex channel objective is a positive multiple of the inward normal
to $q_Z\geq0$, so the convex KKT conditions prove that this facet saddle is
the global optimum over the CPTP triangle.

We next identify its algebraic sheet.  Put $a=2A$ and
$F=64(1+t^2)^2P(\Delta/4;a/2,u)$.  Write
$F=P_2u^2+P_1u+P_0$.  At the saddle $F_u=0$, and direct factorization gives
\[
...
\]
For $t\in[1/4,19/25]$, $\Delta\geq1/2$, and $0\leq a\leq1$, an elementary
rearrangement gives
\[
 -\frac{W_+(a,\Delta,-t)}{(1+t^2)^2}
 \geq\frac{171381}{972196}>0.
\]
Thus the physical saddle lies on $W_+=0$, without numerical sheet selection.

The second stationary equation, after using the linear equation $F_u=0$, is
$G_8(a,\Delta,t)=0$.  If $P_k$ denote the coefficients of $F$ above, it can
be defined without expansion by
\[
 -16G_8=(\partial_aP_2)P_1^2
 -2P_1(\partial_aP_1)P_2+4(\partial_aP_0)P_2^2.
\]
Exact elimination of $a$ gives
\begin{align*}
...
\]
The discriminant is nonzero there.  Hence the root count in $(1/2,2)$ is
constant, and a Sturm count at $t=1/2$ gives one.  Direct substitution of the
weak and strong values produces the squared threshold factors
\[
 (t^6-2t^5-3t^4+7t^2+2t-1)^2,\qquad
 (t^3+t^2-1)^2,
\]
respectively, with strictly positive cofactors.  The middle root therefore
joins both closed branches exactly.
\end{proof}
...
negative depolarizing boundary, the SWAP endpoint, rational points on both
closed XY branches including iSWAP, and the middle-branch quartic controls.
The focused suite is
run from the repository root with
\begin{verbatim}
python3 -m unittest discover -s tests -p 'test_aqc_*.py' -v
\end{verbatim}
The larger symbolic identities in Theorem~\ref{thm:xy-middle} can be
regenerated independently with the optional proof dependencies:
\begin{verbatim}
python3 -m pip install -e '.[proof]'
python3 tools/derive_xy_middle.py
\end{verbatim}
The plotted data are generated without third-party Python dependencies:
\begin{verbatim}
...
of the XY line gives an exact three-regime curve, including the new iSWAP
endpoint $3/2$ and a unique quartic root in the intermediate regime.  These
...
\[
 P_A\big|_{A=1/2,\,\lambda=(1-u)/2}
 =\frac{u(1-u)(1-C)}2>0.
\]
Since $P_\lambda>0$, the largest eigenvalue decreases as $A$ approaches
$1/2$; that endpoint cannot maximize the witness objective.  At $A=0$, the
objective is
\[
 2\sqrt{(u-C^2)^2+C^2S^2}.
\]
Its only stationary point is $u=C^2$.  The derivative into $A>0$ there has
the sign of
\[
 C(1-C)S[2SC-(3C^3+C^2-C-1)],
\]
which is positive exactly after the first threshold.  Thus the middle saddle
has $0<A<1/2$.  Direct endpoint evaluation gives
...
$P(\lambda;A,u)$ vanish identically in $A$, contradicting the coefficient
...
$A$-discriminant polynomial $R$ in the main proof to vanish identically in
$u$, including its leading coefficient $(1-C)^2-8\lambda$.  This contradicts
$4\lambda\geq2SC>1/2$.  Hence both optimizers are unique, and Berge's theorem
...
\[
 A,\ B,\ u,\ 1-u,\ C,\ 1-C^2,\ \lambda,\ P_\lambda,\ \Psi.
\]
Every factor is nonzero: the boundary factors were excluded above;
$0<C<1$ and $\lambda>0$ hold in the middle interval;
$P_\lambda>0$ follows from the simple positive eigenvalue; and
$\Psi>0$ follows from its negative discriminant.  The substitution
$B=(u-C)\lambda/\Psi$ therefore loses no physical solution.  The remaining
generic component $u=-2\lambda$ is incompatible with $u,\lambda>0$.
...
\subsection{Physical sheet and root isolation}
...
saddle.  The resultant in the main proof then forces $Q_+=0$ because all its
remaining factors are strictly positive.

The leading coefficient of $Q_+$ is
\[
 (1+t^2)^2(t^2+t+1)>0.
\]
The endpoint signs and nonzero discriminant displayed in the main proof
prevent roots from entering, leaving, colliding, or escaping to infinity on
the enclosing interval.  The exact Sturm variation difference at $t=1/2$ is
one between $\Delta=1/2$ and $\Delta=2$.  Hence the physical root is unique
throughout the middle regime.
