...
and objective value, and Schur--Weyl duality~\cite{watrous2018} then permits
an optimal input density
\[
...
theorem~\cite{sion1958}.  Berge's maximum theorem~\cite{berge1963} gives
upper hemicontinuity of the optimizer correspondences; when each is
single-valued, this is ordinary continuity.
...
If its unique maximizing witness were $A=0$, Danskin's
theorem~\cite{danskin1967} would give
\[
 f'(u)=\frac{2(u-C^2)}
 {\sqrt{(u-C^2)^2+C^2S^2}},
\]
...
guarantees that no effective channel can do better against
it~\cite{sion1958}.  The witness is
...
\end{center}
For an archived release, the Zenodo landing page records the DOI and immutable
version identifier.  The repository's \texttt{CITATION.cff} is updated with
that DOI after archival; before archival, the repository URL above identifies
the evolving draft.
...
if $A=0$ is active, Danskin's theorem~\cite{danskin1967} identifies the
...
Eq.~\eqref{eq:xy-middle-lower-window}.  Hence both optimizers are unique, and
Berge's theorem~\cite{berge1963} gives their continuity in $C$.
...
\emph{Pacific J. Math.} \textbf{8}, 171 (1958),
\href{https://doi.org/10.2140/pjm.1958.8.171}{DOI}.

\bibitem{berge1963}
C.~Berge,
\emph{Topological Spaces, Including a Treatment of Multi-Valued Functions,
...
\href{https://doi.org/10.1007/978-3-642-87661-6}{DOI}.
