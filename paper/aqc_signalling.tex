...
family and its threshold.  Earlier work characterized causal and semicausal operations
structurally~\cite{beckman2001,eggeling2002}, while continuity of Stinespring
dilations supplies general non-sharp bounds~\cite{ksw2008}.
...
continuous Cartan axis.  The two substantive results are the
...
\cite{ziman2002,scarani2002} and here exhibits the dimension-dependent
...
do not yield Eq.~\eqref{eq:defect}.
...
already obtained in Refs.~\cite{barsse2024,barsse2025}.  The continuous
...
\item[(ii)] \emph{(Concavity and a unique saddle.)}  For fixed $(u,w)$ the
Watrous primal value is concave in the invariant witness parameter
$A\in[0,1/2]$, and its maximizer $A_*(u,w)$ is unique.  Consequently
$F$ is a convex--concave min--max whose value is attained at a saddle, and
by Berge's maximum theorem the saddle depends continuously on $\theta$.
\item[(iii)] \emph{(Sufficiency.)}  Suppose $(u_\star,w_\star)$ lies on the
edge $w=2u-1$, the directional derivative of $F$ along that edge vanishes,
and $F$ admits a subgradient whose normal component points into
$q_Z\geq0$.  Then $(u_\star,w_\star)$ minimizes $F$ over the whole triangle
$\mathcal T$, not merely over the edge.
\end{enumerate}
\end{lemma}
...
density is affine in $A$, giving concavity in $A$.  Uniqueness is proved with
Eq.~\eqref{eq:xy-middle-characteristic} below: a maximizing interval would
force the cubic $P(\ell;A,u)$ to vanish identically in $A$, contradicting the
leading coefficient $-\Theta$.  A convex--concave function on a product of
compact convex sets has a saddle by Sion's minimax theorem, and the
maximum theorem gives continuity of the saddle correspondence; single-valued
continuity follows from the uniqueness just established.
...
whose normal component points inward means that some subgradient lies in the
normal cone of $\mathcal T$ at $(u_\star,w_\star)$, i.e.\
...
by Lemma~\ref{lem:facet}(i)--(ii); the uniqueness of the maximizing $A$ is the
argument promised there, namely that an interval of maximizing $A$ values
would make the cubic $P(\ell;A,u)$ vanish identically despite its leading
coefficient $-\Theta$.
...
Constancy on an interval would force $\mathcal R$ to vanish identically in $u$, hence
$4\ell=(1-C)^2/2<1/2$, whereas the $A=0$ witness gives
$4\ell\geq2SC>1/2$ by
Eq.~\eqref{eq:xy-middle-lower-window}.  The facet saddle is therefore unique and varies
continuously.
...
Lemma~\ref{lem:facet}(ii) the facet saddle varies continuously in $\theta$, so
$G$ can change sign on the open interval only by vanishing there, and the
elimination above shows that this happens only at $\theta_2$.  The gradient of
the convex channel objective is then a positive multiple of the inward normal
to $q_Z\geq0$: tangential stationarity gives zero directional derivative along
the facet, while $G>0$ supplies a subgradient with inward normal component.
...
gates belonging to different local-equivalence classes.  By the universal
bound~\eqref{eq:universal-swap-bound}, iSWAP is therefore a maximally
...
of that state improves the worst-case prediction.
...
cross-check are openly available in the repository cited in the reproducibility
commands above, together with pinned dependency versions.  Software is released
